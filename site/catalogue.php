...
while ($row = $result->fetch_assoc()) {
	...
	if (isset($CART_ITEMS)) {
		...
		$total_price += $CART_ITEMS[$cart_item_id]['price'] * intval($CART_ITEMS[$cart_item_id]['amount']);
	}
...
<form class="payment_btn" id="rideeffect" name="paypal" action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_top">
	<table class="buy" style="display: table; margin: 0 auto;">
		<tr>
			<td colspan="2" align="center">
				<h1 class="price">
					<b id="item_show_price_total"><?=$total_price?></b>
				</h1>
				<h5 class="curr"><?=$DICT['currency']?></h5>
			</td>
			<td></td>
	...
	foreach ($CART_ITEMS as $cart_item_id => $cart_item) { 
		...
		<input type="hidden" name="amount_<?=$i?>" value="<?=$cart_item['price']?>">
		<input type="hidden" name="quantity_<?=$i?>" value="<?=$cart_item['amount']?>">
	<?php } ?>
